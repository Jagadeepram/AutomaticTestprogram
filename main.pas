...
//  Sleep(100);
  ...
  if (fwVersion >= 5.2) then begin
    ...
    //      OceanLicens    : 11b Oregistrerad      (Enligt spec från Baracuda/Pirahna)
    //                       10b Display
    //                       01b Connect
    //                       00b Professional
...
begin
  ...
  lstItem.Caption := artStr;                          // Artikelbenämning
  ...
  lstItem.SubItems.Add(Format('%s'+RTIIS_REP_SEP+'%s'+RTIIS_REP_SEP+'%s',           // Inmätning för beräkning kalibrering: high
  ...
  lstItem.SubItems.Add(Format('%s'+RTIIS_REP_SEP+'%s'+RTIIS_REP_SEP+'%s',           // Inmätning för beräkning kalibrering: low
  ...
  lstItem.SubItems.Add(Format('%.1f'+RTIIS_REP_SEP+'%.3f'+RTIIS_REP_SEP+'%.2f'+
                              RTIIS_REP_SEP+'%.1f'+RTIIS_REP_SEP+'%.0f'+ RTIIS_REP_SEP, // Laddning uppmätt
  ...
  lstItem.SubItems.Add('750,0'+RTIIS_REP_SEP+'7,500'+RTIIS_REP_SEP+'75,00'+RTIIS_REP_SEP+
                       '750,0'+RTIIS_REP_SEP+'7500,0'+RTIIS_REP_SEP);                        // Ström referens
  lstItem.SubItems.Add('pA'+RTIIS_REP_SEP+'nA'+RTIIS_REP_SEP+'nA'+RTIIS_REP_SEP+
                       'nA'+RTIIS_REP_SEP+'nA'+RTIIS_REP_SEP);                                // Enhet ström
  lstItem.SubItems.Add(Format('%.1f'+RTIIS_REP_SEP+'%.3f'+RTIIS_REP_SEP+
                              '%.2f'+RTIIS_REP_SEP+'%.1f'+RTIIS_REP_SEP+'%.0f'+RTIIS_REP_SEP, // Ström uppmätt
...
begin
  ...
  lstItem.SubItems.Add(Format('%s'+RTIIS_REP_SEP+'%s'+RTIIS_REP_SEP+'%s',           // Inmätning för beräkning kalibrering: high sens
  ...
  lstItem.SubItems.Add(Format('%s'+RTIIS_REP_SEP+'%s'+RTIIS_REP_SEP+'%s',           // Inmätning för beräkning kalibrering: low sens
  ...
  lstItem.SubItems.Add('');                                                         // Laddning uppmätt
  lstItem.SubItems.Add(Format('%.2f'+RTIIS_REP_SEP+'%.2f'+RTIIS_REP_SEP+
                              '%.1f'+RTIIS_REP_SEP+'%.1f'+RTIIS_REP_SEP,                 // Ström referens
  ...
  lstItem.SubItems.Add('mA'+RTIIS_REP_SEP+'mA'+RTIIS_REP_SEP+'mA'+RTIIS_REP_SEP+
                       'mA'+RTIIS_REP_SEP);                                              // Enhet ström
  lstItem.SubItems.Add(Format('%.2f'+RTIIS_REP_SEP+'%.2f'+RTIIS_REP_SEP+
                              '%.1f'+RTIIS_REP_SEP+'%.1f'+RTIIS_REP_SEP,                 // Ström uppmätt
...
var
  s                : string;
  I,J              : Integer;
  words            : array [0..20] of string;{ TODO : Gör om till en TStringList, den är inte begränsad till 21 utan dynamisk. }
